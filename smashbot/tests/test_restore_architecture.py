...
loudly: gate_gelu and v_norm change behavior but no parameter shape, so the
...
def _config(run_dir: str, steps: int, gate_gelu: bool) -> train_bc.TrainConfig:
    return train_bc.TrainConfig(
        ...
        network=configs.NetworkConfig(
            name="sgu", hidden_size=32, num_layers=1, window=8,
            attn_heads=2, attn_head_dim=8, gate_gelu=gate_gelu, v_norm=gate_gelu,
        ),
...
def test_restore_without_the_paper_settings_is_refused(tmp_path):
    train_bc.main(_config(str(tmp_path), 1, gate_gelu=True))
    forgot = _config(str(tmp_path), 2, gate_gelu=False)
    forgot.runtime.restore = "auto"
    with pytest.raises(ValueError, match="gate_gelu=True"):
        train_bc.main(forgot)
    same = _config(str(tmp_path), 2, gate_gelu=True)
...
def test_checkpoints_that_predate_a_setting_take_its_default():
    ...
    current.network.gate_gelu = True
...
def test_league_refuses_a_student_config_with_weightless_settings():
    ...
    league._cfg = {"network": dataclasses.asdict(configs.NetworkConfig(name="sgu", v_norm=True))}
    with pytest.raises(NotImplementedError, match="v_norm"):
