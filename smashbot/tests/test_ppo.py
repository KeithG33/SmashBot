...
def test_dclamp_surrogate():
    """DClamp third term (arXiv:2511.02577): active ONLY in the strict
    wrong-direction region, steeper slope there, exact PPO elsewhere."""
    eps, a, b = 0.01, 2.0, 0.02

    def surr(lr, adv):
        t = torch.tensor([lr], requires_grad=True)
        out = clipped_surrogate(t, torch.tensor([adv]), eps, a, b)
        out.sum().backward()
        return out.item(), t.grad.item()

    base = lambda lr, adv: clipped_surrogate(
        torch.tensor([lr]), torch.tensor([adv]), eps
    ).item()

    # near 1 and in the right direction: identical to plain PPO
    for lr, adv in [(0.0, 1.0), (0.005, 1.0), (-0.005, -1.0), (0.03, 1.0)]:
        val, _ = surr(lr, adv)
        assert val == pytest.approx(base(lr, adv), abs=1e-6)

    # strict wrong direction, A>0 (ratio well below 1-b): penalized & steeper
    lr_bad = float(np.log(0.9))
    val, grad = surr(lr_bad, 1.0)
    assert val < base(lr_bad, 1.0)
    # d/dlog_r of a*r*A = a*r*A = 2*0.9 -> steeper than PPO's r*A = 0.9
    assert grad == pytest.approx(2 * 0.9, abs=1e-4)

    # strict wrong direction, A<0 (ratio well above 1+b): penalized & steeper
    lr_bad = float(np.log(1.1))
    val, grad = surr(lr_bad, -1.0)
    assert val < base(lr_bad, -1.0)
    assert grad == pytest.approx(-2 * 1.1, abs=1e-4)

    # alpha=0 disables: exact equality everywhere
    lrs = torch.randn(50) * 0.1
    advs = torch.randn(50)
    assert torch.equal(
        clipped_surrogate(lrs, advs, eps, 0.0),
        clipped_surrogate(lrs, advs, eps),
    )
