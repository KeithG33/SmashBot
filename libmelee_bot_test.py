...
def gamestate_to_model_input(gamestate, player_port_index, in_game=True):
    """
    Converts gamestate to model input. 
    """
    obs, controllerstate = parse_game_state(gamestate, in_game=in_game)
    ...
    return player_input_tensor, controllerstate
...
checkpoint = '/home/kage/smashbot_workspace/SmashBotTransformer-BC.pt'
...
while True:
    ...
    if gamestate.menu_state in [melee.Menu.IN_GAME, melee.Menu.SUDDEN_DEATH]:
        ...
        if done: 
            ...
            break
        player_input_tensor, _ = gamestate_to_model_input(gamestate, CPU_PORT-1)
        player_input_tensor2, human_actions = gamestate_to_model_input(gamestate, HUMAN_PORT-1, in_game=False)

        with torch.inference_mode():
            player_input_tensor = player_input_tensor.cuda()
            player_input_tensor2 = player_input_tensor2.cuda()
            output = model.fc1(player_input_tensor)
            output2 = model.fc1(player_input_tensor2)
